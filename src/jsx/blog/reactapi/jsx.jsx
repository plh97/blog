  ...
  render() {
    return (
      <Col span={24}>
        <h3>介绍JXP</h3>
        <p>接下来看看下面这个声明:</p>
        <pre><code className="language-javascript">{`const element = <h1>Hello World!;</h1>`}</code></pre>
...
ReactDOM.Render(
  ...
  <p>我们将JSX分割成多行，使其更加语义化。虽然这是不必要的，但我们仍然建议将他写在一个<code className="language-javascript">{'{}'}</code>里面，用来避免自动插入分号的陷阱。</p>
  <h4>JSX也是一个表达式</h4>
  <p>编译后，JSX表达式变成常规的js对象</p>
  <p>这意味着你可以在if语句for循环中使用JSX，给他传入变量，或者参数，并从函数中返回。</p>
  <h4>用JSX指定属性</h4>
  <p>maybe you can 用引号为字符串中文字指定属性。</p>
  <pre><code className="language-javascript">{`const element = <div tabindex="0"></div>`}</code></pre>
  <p>你也可以将js表达式用花括号包住嵌入到一个属性中</p>
  <pre><code className="language-javascript">{`const element = <img src={user.avatarUrl} />`}</code></pre>
  <p>当你想要将js表达式嵌入到一个属性中，不要<code className="language-javascript">{'{}'}</code>或者<code className="language-javascript">{'{}'}</code>这样，因为JSX会将这个属性原封不动的当作变量而不是一个表达式，你要么用''表示字符串值，要么用<code className="language-javascript">{'{}'}</code>表示js表达式，但是不要一起用在一个属性中。</p>
  <h4>用JSX指定子元素</h4>
