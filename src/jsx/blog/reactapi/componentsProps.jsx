  ...
  render() {
    ...
    return (
      <Col span={24}>
        ...
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function Welcome(props){
          return <h1>Hello, {props.name}</h1>;
        }
        `}</SyntaxHighlighter>
        <p>这个函数是一个有效的React组件因为它可以接受一个单一的'props'对象，参数携带数据并且返回一个React元素，我们叫这样的组件为函数化因为他是真真的js函数。</p>
        <p>你也可以用ES6 class来定义一个组件:</p>
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`class Welcome extends React.Component {
          render(){
        ...
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function Comment(prop){
          return(
        ...
        <p>首先我们会提取Avatar组件，</p>
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function Avatar(props){
          return (
            <img
              src={props.author.avatarUrl}
              alt={prop.author.name}
        ...
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function UserInfo(props){
          return(
            <div className="UserInfo">
        ...
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function Comment(props){
          return(
            <div className="Comment">
        ...
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function sum(a,b){
          return a+b;
        }
        `}</SyntaxHighlighter>
        <p>这叫做纯函数'pure'，因为他们不会去改变他们的输入值，并且在同样参数的情况下会输出同样的结果。</p>
        <p>下面例子刚好相反，因为他改变了他自己的输入。</p>
        <SyntaxHighlighter showLineNumbers={showLineNumbers} language={language} style={style}>{`function withdraw(account,amount){
