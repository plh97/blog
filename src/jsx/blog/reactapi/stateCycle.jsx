  ...
  render() {
    return (
      <Col span={24}>
        ...
        <p>我们调用<code className="language-javascript">ReactDOM.render()</code>来改变那些已经渲染过的输出。</p>
...
`ReactDOM.render(
  ...
  document.getElementById('example')
        ...
        <ol type="1">
        <li>创造一个ES6的class，名字字样，然后加上<code className="language-javascript">extends React.Component.</code></li>
        <li>增加一个单一的空的方法，让他可以调用render()</li>
        <li>将所有div元素移入<code className="language-javascript">render()</code>里面。</li>
        <li>将<code className="language-javascript">props</code>替换为<code className="language-javascript">this.props</code>,并且放置在<code className="language-javascript">render()</code>里面。</li>
        <li>删除空的<code className="language-javascript">function</code>声明。</li>
        </ol>
        <pre><code className="language-javascript">{
  ...
  render(){
    return(
      ...
      &le;/div>
        ...
        <p>1)将<code className="language-javascript">this.props.date</code>替换为<code className="language-javascript">this.state.date</code>,在<code className="language-javascript">render()</code>方法里面：</p>
        <pre><code className="language-javascript">{
`class Clock extends React.Component {
  render(){
    return(
      <div>
        ...
        <p>2)添加一个类的构造函数并且指定初始化值<code className="language-javascript">this.state,super(props)</code>出现的目的是为了this这个值能够使用，在子类constructor中必须先调用super才能调用this。</p>
        <pre><code className="language-javascript">{
`class Clock extends React.Component{
  constructor(props){
    super(props);
  ...
  render(){
    return (
      <div>
        ...
        <p><code className="language-javascript">componentDidMount()</code>钩子在组件输出渲染到DOM之后运行，这是一个设置解释器的好地方。</p>
        <pre><code className="language-javascript">{
`componentDidMount(){
...
ReactDOM.render(
  ...
  document.getElementById("example")
        ...
        <p>1)当<code className="language-html">&lt;CLock/></code>被传递给<code className="language-javascript">ReactDOM.render()</code>时,React调用Clock组件构造器，知道Clock需要显示当前时间，它初始化this.state通过一个对象包括当前时间，我们之后就会更新这个state。</p>
        <p>2)React接着会调用Clock组件的render()方法，这就是React如何得知什么东西应该显示在屏幕上面。React接着会更新DOM来匹配Clock的渲染输出。</p>
        <p>3)当Clock输出并且被嵌入到DOM中，React调用<code className="language-javascript">componentDidMount()</code>lifecycle hook，其中，Clock组件请求浏览器来加载一个计时器来时刻调用tick()</p>
